...
//Jika kredensial yang diberikan oleh pengguna cocok dengan data yang tersedia di database, maka kita akan mencetak string "login_success"
//String "login_success" akan dikembalikan ke fungsi anonim yang memanggil $("#login").click()
...
if(isset($_POST["email"]) && isset($_POST["password"])){
	...
	if($count == 1){
			...
			// Jika pengguna melakukan login setelah halaman keranjang, kita akan mengirimkan "cart_login"
				echo "cart_login";
         ...
         // Jika data pengguna tersedia di database, maka $count akan sama dengan 1
            if($count == 1){
                $row = mysqli_fetch_array($run_query);
                $_SESSION["uid"] = $row["admin_id"];
                $_SESSION["name"] = $row["admin_name"];
                $ip_add = getenv("REMOTE_ADDR");
               // saya telah membuat cookie di halaman login_form.php, jadi jika cookie tersebut tersedia berarti pengguna belum login
